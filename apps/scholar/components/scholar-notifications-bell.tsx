...
import {
  useMarkScholarNotificationsRead,
  useScholarNotificationsFeed,
...
function notificationIcon(kind: ScholarNotificationKind) {
  switch (kind) {
    case 'task_assigned':
      return CheckSquare;
    case 'resource_live':
...
function NotificationsPanel({ onNavigate }: { onNavigate: () => void }) {
  ...
  const { data, isLoading, isFetching, isFetchingNextPage, hasNextPage, fetchNextPage } =
    useScholarNotificationsFeed(deferredSearch);
  const markRead = useMarkScholarNotificationsRead();

  const items = data?.pages?.flatMap((page) => page.items) ?? [];
  const unreadCount = data?.pages?.[0]?.unreadCount ?? 0;

  const handleItemClick = async (notification: ScholarNotification) => {
    onNavigate();
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          unread
                            ? 'border-ashinaga-teal-400 bg-ashinaga-teal-50 text-ashinaga-teal-700 dark:border-ashinaga-teal-700 dark:bg-ashinaga-teal-950 dark:text-ashinaga-teal-300'
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="flex items-start justify-between gap-2">
                          ...
                          {unread ? (
                            <span
                              className="mt-1 size-2 shrink-0 rounded-full bg-ashinaga-teal-600"
                              title="Unread"
                            />
                          ) : null}
                        </span>
...
function BellButton({ unreadCount, className }: { unreadCount: number; className?: string }) {
  return (
    <span className={cn('relative inline-flex', className)}>
      ...
      {unreadCount > 0 ? (
        <span className="absolute -right-2 -top-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-ashinaga-teal-600 px-1 text-[10px] font-semibold text-white">
...
export function ScholarNotificationsBell() {
  const { isMobile } = useSidebar();
  const { data } = useScholarNotificationsFeed();
  const unreadCount = data?.pages?.[0]?.unreadCount ?? 0;
